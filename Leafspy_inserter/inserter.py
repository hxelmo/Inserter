...
from config import SERVER, DATABASE, ToinsertPath, InsertedPath, EndDate_result, StartDate_result, Insert_Table
import sys
import csv
import pyodbc
import shutil
...
def search_filelist():
    ...
    """
    p = Path(path.join(path.dirname(__file__), ToinsertPath))
    ...
    for file in p.iterdir():
        ...
        if re.match('(Log_).+[0-9]{8}(.csv)', file.name):
            ...
            if (StartDate_result < file_date_2) & (EndDate_result > file_date_2):
...
def move_file(filename):
    shutil.move(filename,InsertedPath)

def copy_file(filename):
    shutil.copy2(filename,InsertedPath)
