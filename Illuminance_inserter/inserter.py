...
from config import SERVER, DATABASE, TABLE_NAME, ToinsertPath, InsertedPath, carID, driverID, sensorID, StartDate, EndDate
import sys
import csv
import pyodbc
import re
        ...
        with pyodbc.connect(config) as connection:
            with connection.cursor() as cursor:
                ...
                for row in csv.DictReader(csvfile, csv_header):  #rowは辞書(dict)形式
                    #print('row2:' + str(row))
                    insert_bool = 0
                    illuminance_record = IlluminanceRecord(row) #これをカンマで分割して値をとろう
                    ...
                    stmt = """
                        INSERT INTO {} {} 
                        VALUES {}
                        """.format(TABLE_NAME, IlluminanceRecord.column_list(), str(illuminance_record))
